...
export class SCSDKImportService {
     ...
     * @returns ImportTask
     */
    public create(createImportDto: CreateImportDTO): Observable<ApiResponse<ImportTask | null>> {
        if (typeof createImportDto === "undefined" || createImportDto == null) return of(ApiResponse.withPayload(null));
    ...
    public findPendingByUser(userId: string, pageable: Pageable): Observable<ApiResponse<Page<ImportTask>>> {
        if (!userId) return of(ApiResponse.withPayload(Page.empty()));
    ...
    public deleteById(taskId: string): Observable<ApiResponse<boolean>> {
        if (!taskId) return of(ApiResponse.withPayload(false));
     ...
     */
    public findReportByTaskid(taskId: string): Observable<ApiResponse<ImportReport | null>> {
        if (!taskId) return of(ApiResponse.withPayload(null));
