 ...
 */
export class SCResourceMapQueue<T extends { id?: string }> {
    ...
    public enqueue(item: T): number {
        if (!item["id"]) item["id"] = randomString(254);
        this.set(item);
     ...
     * @returns Item
     */
    public async dequeue(random: boolean = false): Promise<T | null> {
        // Peek next element to check if there is any item left.
        const peek = await this._queue.peek(random) as any
        if (!peek || !peek.item || !peek.item) return null;

        // Dequeue the peeked item.
        this._queueMap.remove(peek.item.id);
        const item = await this._queue.dequeuePosition(peek.position) as any;
