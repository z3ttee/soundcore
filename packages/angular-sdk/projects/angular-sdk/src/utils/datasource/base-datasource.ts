...
export abstract class SCSDKBaseDatasource<T = any> extends DataSource<T> {
    ...
    protected dataStream = new BehaviorSubject<(T | null)[]>(this.cachedData ?? []);
    ...
    constructor(
        ...
        this.cachedData = Array.from({ length: this.initialSize })
    ...
    protected getPageOrFetch(pageIndex: number): Observable<T[]> {
        return new Observable((subscriber) => {
            if (this.hasPage(pageIndex)) {
                ...
                subscriber.next(elements as any);
                subscriber.complete();
                return;
            }
