...
const OPACITY_END_CLASS = "opacity-100";
...
export class SCNGXTooltipDirective implements OnDestroy, OnInit {
  ...
  private onMouseEnter(event: MouseEvent) {
    this._tooltip.classList.remove(SCALE_BEGIN_CLASS, OPACITY_BEGIN_CLASS)
    this._tooltip.classList.add(SCALE_END_CLASS, OPACITY_END_CLASS)
  }

  private onMouseLeave(event: MouseEvent) {
    this._tooltip.classList.remove(SCALE_END_CLASS, OPACITY_END_CLASS)
    this._tooltip.classList.add(SCALE_BEGIN_CLASS, OPACITY_BEGIN_CLASS)
  }
  ...
  private prepareTooltip(): HTMLElement {
    ...
    // Add initial styling
    tooltip.classList.add("transition-all", "transform-gpu", "will-change-transform", "pointer-events-none", SCALE_BEGIN_CLASS, OPACITY_BEGIN_CLASS)
