...
export class MeiliClient extends MeiliSearch {
    ...
    public override index<T extends Record<string, any> = Record<string, any>>(indexUid: string): MeiliIndex<T> {
        return createMeiliIndex(this.config, this.schemas.get(indexUid)) as MeiliIndex<any>;
    }

    public override async getIndex<T extends Record<string, any> = Record<string, any>>(indexUid: string): Promise<MeiliIndex<T>> {
        return this.index<T>(indexUid).fetchInfo()
    }

    public override async getIndexes(parameters?: Pagination): Promise<IndexesResults<MeiliIndex[]>> {
